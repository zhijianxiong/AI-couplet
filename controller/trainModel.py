...
class Model():
    def __init__(self,train_input_file, train_target_file,
        ...
        if init_train:
            self.train_reader = seqReader.SeqReader(train_input_file,
                                                    train_target_file, vocab_file, batch_size)
            self.train_reader.start()
            self.train_data = self.train_reader.read()
            self.eval_reader = seqReader.SeqReader(test_input_file, test_target_file,
        ...
        if init_infer:
            self.infer_vocabs = seqReader.read_vocab(vocab_file)
        ...
        with self.train_graph.as_default():
            ...
            for step in range(start, epochs):
                ...
                if step % self.save_step == 0:
                    l = total_loss / self.save_step
                    self.list_loss.append(l)
                    ...
                    input_text = seqReader.decode_text(in_seq[sid],
                                                       self.eval_reader.vocabs)
                    output_text = seqReader.decode_text(output[sid],
                                                        self.train_reader.vocabs)
                    target_text = seqReader.decode_text(target_seq[sid],
        ...
        with self.eval_graph.as_default():
            ...
            for step in range(0, self.eval_reader.data_size):
                ...
                for i in range(len(outputs)):
                    output = outputs[i]
                    target = target_seq[i]
                    output_text = seqReader.decode_text(output,
                                                        self.eval_reader.vocabs).split(' ')
                    target_text = seqReader.decode_text(target[1:],
                                                        self.eval_reader.vocabs).split(' ')
                    ...
                    if random.randint(1, prob) == 1:
                        print('====================')
                        input_text = seqReader.decode_text(in_seq[i],
                                                           self.eval_reader.vocabs)
                        print('src:' + input_text)
                        print('output: ' + ' '.join(output_text))
                        print('target: ' + ' '.join(target_text))
            return count.compute_bleu(target_results, output_results)[0] * 50
    ...
    def infer(self, text):
        ...
        with self.infer_graph.as_default():
            in_seq = seqReader.encode_text(text.split(' ') + ['</s>', ],
            ...
            output = outputs[0]
            output_text = seqReader.decode_text(output, self.infer_vocabs)
            return output_text
